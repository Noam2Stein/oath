...
pub enum Expr {
    ...
    ShsOp(#[span] Span, oath_ast::UnaryOperator, Try<Box<Expr>>),
    MhsOp(#[span] Span, Box<Expr>, oath_ast::BinaryOperator, Try<Box<Expr>>),
}
...
impl Expr {
    pub fn from_syntax(
        ...
        match ast_expr {
            oath_ast::Expr::Ident(ident) => {
                Self::Name(ident.span(), name_context.namespace(namespace).try_name_id(ident, context))
            }
            ...
            oath_ast::Expr::Tuple(span, items) => Self::Tuple(
                ...
                items
                    .into_iter()
                    .map(|item| item.map(|item| Self::from_syntax(item, namespace, name_context, context)))
                    .collect(),
            ),
            ...
            oath_ast::Expr::Field(base, field) => {
                Self::Field(Box::new(Self::from_syntax(*base, namespace, name_context, context)), field)
            }
            oath_ast::Expr::Index(span, base, index) => Self::Index(
                span,
                Box::new(Self::from_syntax(*base, namespace, name_context, context)),
                index.map(|index| Box::new(Self::from_syntax(*index, namespace, name_context, context))),
            ),
            oath_ast::Expr::Call(span, base, args) => Self::Call(
                span,
                Box::new(Self::from_syntax(*base, namespace, name_context, context)),
                args.into_iter()
                    .map(|arg| arg.map(|arg| Self::from_syntax(arg, namespace, name_context, context)))
                    .collect(),
            ),
            oath_ast::Expr::Generics(base, generics) => {
                Self::Generics(Box::new(Self::from_syntax(*base, namespace, name_context, context)), generics)
            }
            oath_ast::Expr::ShsOp(span, op, base) => Self::ShsOp(
                span,
                op,
                base.map(|base| Box::new(Self::from_syntax(*base, namespace, name_context, context))),
            ),
