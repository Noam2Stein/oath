...
pub trait FromRegexStr {
    fn from_regex_str(span: Span, str: &str, path: &Path, interner: &Interner, diagnostics: &Diagnostics) -> Self;
}
...
impl FromRegexStr for LiteralSuffix {
    fn from_regex_str(span: Span, str: &str, path: &Path, interner: &Interner, diagnostics: &Diagnostics) -> Self {
        match Ident::new(str, span, &interner) {
            ...
            None => Self {
                ident: Try::Failure(Some(
                    diagnostics.push_error(path.to_path_buf(), SyntaxError::Expected(span, "an ident")),
                )),
            },
        }
    }
...
impl FromRegexStr for IntLiteral {
    fn from_regex_str(span: Span, str: &str, path: &Path, interner: &Interner, diagnostics: &Diagnostics) -> Self {
        ...
        let value = u128::from_str_radix(value_str, 10).map_or_else(
            |_| {
                Try::Failure(Some(
                    diagnostics.push_error(path.to_path_buf(), TokenError::OutOfBoundsLiteral(span)),
                ))
            },
            |value| Try::Success(value),
        );

        let suffix = suffix_str.map(|suffix_str| LiteralSuffix::from_regex_str(span, suffix_str, path, interner, diagnostics));
    ...
    fn from_regex_str(span: Span, str: &str, path: &Path, interner: &Interner, diagnostics: &Diagnostics) -> Self {
        ...
        let dot_position = match dot_position {
            ...
            _ => {
                ...
                let fractional = Try::Failure(Some(
                    diagnostics.push_error(path.to_path_buf(), SyntaxError::Expected(span, "`_._`")),
                ));
...
impl FromRegexStr for StrLiteral {
    fn from_regex_str(span: Span, str: &str, _path: &Path, interner: &Interner, _diagnostics: &Diagnostics) -> Self {
        Self {
...
impl FromRegexStr for CharLiteral {
    fn from_regex_str(span: Span, str: &str, _path: &Path, _interner: &Interner, _diagnostics: &Diagnostics) -> Self {
        Self {
