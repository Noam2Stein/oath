...
pub fn impl_trait(input: &DeriveInput, trait_ident: &'static str, items: impl IntoIterator<Item = TokenStream>) -> TokenStream {
...
pub fn data_split<O>(
    data: &Data,
    attrs: &[Attribute],
    struct_: impl FnOnce(&DataStruct, &[Attribute]) -> O,
    enum_: impl FnOnce(&DataEnum, &[Attribute]) -> O,
) -> O {
    match data {
