...
pub trait FromRegexStr {
    fn from_regex_str(span: Span, str: &str, context: &mut ParseContext) -> Self;
}
...
impl FromRegexStr for IntLiteral {
    fn from_regex_str(span: Span, str: &str, context: &mut ParseContext) -> Self {
        ...
        let suffix = suffix_str.map_or(None, |suffix_str| {
            Ident::new(suffix_str, span, &context.interner).or_else(|| {
...
impl FromRegexStr for CharLiteral {
    fn from_regex_str(span: Span, str: &str, _context: &mut ParseContext) -> Self {
        Self {
